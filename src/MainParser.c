...
/* External objects */
extern phonon_intg syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern phonon_intg line; /* source code line number - defined in scanner.c */
...
extern phonon_void startParser(phonon_void);
extern phonon_intg startScanner(ReaderPointer sc_buf);

static phonon_void printParserError(phonon_char* fmt, ...);
static phonon_void displayParser(ReaderPointer ptrBuffer);
static phonon_long getParserFileSize(phonon_char* fname);
static phonon_void callGarbageCollector(phonon_void);
...
phonon_intg mainParser(phonon_intg argc, phonon_char** argv) {
...
phonon_void printParserError(phonon_char* fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(phonon_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
phonon_long getParserFileSize(phonon_char* fname) {
	FILE* input;
	phonon_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
phonon_void displayParser(ReaderPointer ptrBuffer) {
...
phonon_void callGarbageCollector(phonon_void) {
