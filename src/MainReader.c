 ...
 * -------------------------------------------------------------
 */
phonon_void bErrorPrint(phonon_char* fmt, ...);
phonon_void displayBuffer(BufferReader* ptr_Buffer);
phonon_long getFileSize(phonon_char* fname);
phonon_intg isNumber(const phonon_char* ns);
phonon_void startReader(phonon_char*, phonon_char*, phonon_char, phonon_intg, phonon_intg);
...
phonon_intg mainReader(phonon_intg argc, phonon_char** argv) {
	...
	phonon_char* program = argv[0];
	phonon_char* input = argv[2];
	phonon_char mode = MODE_FIXED;
	phonon_intg size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
phonon_void startReader(phonon_char* program, phonon_char* input, phonon_char mode, phonon_intg size, phonon_intg increment) {

	ReaderPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	phonon_intg loadSize = 0;		/* the size of the file loaded in the buffer */
	phonon_char symbol;			/* symbol read from input file */
