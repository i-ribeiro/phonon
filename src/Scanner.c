...
extern phonon_intg transitionTable[][TABLE_COLUMNS];
...
Token tokenizer(phonon_void) {
	...
	phonon_char c;	/* input symbol */
	phonon_intg state = 0;		/* initial state of the FSM */
	phonon_intg lexStart;		/* start offset of a lexeme in the input char buffer (array) */
	phonon_intg lexEnd;		/* end offset of a lexeme in the input char buffer (array)*/

	phonon_intg lexLength;		/* token length */
	phonon_intg i;				/* counter */
	phonon_char newc;			/* new char */
	...
	while (1) { /* endless loop broken by token returns it will generate a warning */
		...
		*/
		...
		default: // general case
			...
			lexemeBuffer = readerCreate((phonon_intg)lexLength + 2, 0, MODE_FIXED);
			if (!lexemeBuffer) {
...
phonon_intg nextState(phonon_intg state, phonon_char c) {
	phonon_intg col;
	phonon_intg next;
	col = nextClass(c);
...
phonon_intg nextClass(phonon_char c) {
	phonon_intg val = -1;
	switch (c) {
	case CHRCOL2:
		val = 2;
...
Token funcSL(phonon_char lexeme[]) {
	Token currentToken = { 0 };
	phonon_intg i = 0, len = (phonon_intg)strlen(lexeme);
...
phonon_void printToken(Token t) {
	...
	/* External variables */
	extern phonon_char* keywordTable[]; /* link to keyword table in */
